...
public class PositiveIntegerData extends IntData {
	...
	public ArrayList<String> constraintAnalysis() {
		...
		String pattern = null;
		if(constraint == null){
			values = dataGeneration(pattern,1, Integer.MAX_VALUE - 1);
		}else{
			if (constraint.containsKey(Constraints.pattern.toString())) {
				...
				if (constraint.containsKey(Constraints.minExclusive.toString())) {
					if (constraint.containsKey(Constraints.maxExclusive.toString())) {
						...
						values = dataGeneration(pattern,Integer.parseInt(constraint.get(Constraints.minExclusive.toString())) + 1, Integer.MAX_VALUE - 1);
					}
					...
					if (constraint.containsKey(Constraints.maxExclusive.toString())) {
						...
						values = dataGeneration(pattern,Integer.parseInt(constraint.get(Constraints.minInclusive.toString())), Integer.MAX_VALUE - 1);
					}
				} else if (constraint.containsKey(Constraints.maxExclusive.toString())) {
						values = dataGeneration(pattern,1, Integer.parseInt(constraint.get(Constraints.maxExclusive.toString())) - 1);
				} else if (constraint.containsKey(Constraints.maxInclusive.toString())) {
						values = dataGeneration(pattern,1, Integer.parseInt(constraint.get(Constraints.maxInclusive.toString())));
				} else {
					values = dataGeneration(pattern,1, Integer.MAX_VALUE - 1);
				}
			}else{
