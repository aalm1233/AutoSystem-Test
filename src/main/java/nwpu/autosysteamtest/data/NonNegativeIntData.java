...
public class NonNegativeIntData extends IntData {
	...
	public ArrayList<String> constraintAnalysis() {
		...
		String pattern = null;
		if (constraint == null) {
			values = dataGeneration(pattern,0, Integer.MAX_VALUE - 1);
		} else {
			if (constraint.containsKey(Constraints.pattern.toString())) {
				...
				if (constraint.containsKey(Constraints.minExclusive.toString())) {
					...
					values = dataGeneration(pattern,0, Integer.parseInt(constraint.get(Constraints.maxExclusive.toString())) - 1);
				} else if (constraint.containsKey(Constraints.maxInclusive.toString())) {
					values = dataGeneration(pattern,0, Integer.parseInt(constraint.get(Constraints.maxInclusive.toString())));
				} else {
					values = dataGeneration(pattern,0, Integer.MAX_VALUE - 1);
				}
			}else{
