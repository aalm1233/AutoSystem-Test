	...
	public ArrayList<String> constraintAnalysis() {
		...
		if (constraint.containsKey(Constraints.minExclusive.toString())) {
			if (constraint.containsKey(Constraints.maxExclusive.toString())) {
				...
				values = dataGeneration(Long.parseLong(constraint.get(Constraints.minExclusive.toString())) + 1L, Long.MAX_VALUE - 1L);
			}
			...
			if (constraint.containsKey(Constraints.maxExclusive.toString())) {
				...
				values = dataGeneration(Long.parseLong(constraint.get(Constraints.minInclusive.toString())), Long.MAX_VALUE - 1L);
			}
		} else if (constraint.containsKey(Constraints.maxExclusive.toString())) {
			values = dataGeneration(Long.MIN_VALUE + 1L, Long.parseLong(constraint.get(Constraints.maxExclusive.toString())) - 1L);
		} else if (constraint.containsKey(Constraints.maxInclusive.toString())) {
			values = dataGeneration(Long.MIN_VALUE + 1L, Long.parseLong(constraint.get(Constraints.maxInclusive.toString())));
		} else {
