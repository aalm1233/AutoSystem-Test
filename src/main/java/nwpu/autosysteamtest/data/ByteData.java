	...
	public ArrayList<String> constraintAnalysis() {
		...
		if (constraint.containsKey(Constraints.minExclusive.toString())) {
			if (constraint.containsKey(Constraints.maxExclusive.toString())) {
				...
				values = dataGeneration(Byte.parseByte(constraint.get(Constraints.minExclusive.toString())) + 1, Byte.MAX_VALUE - 1);
			}
			...
			if (constraint.containsKey(Constraints.maxExclusive.toString())) {
				...
				values = dataGeneration(Byte.parseByte(constraint.get(Constraints.minInclusive.toString())), Byte.MAX_VALUE - 1);
			}
		} else if (constraint.containsKey(Constraints.maxExclusive.toString())) {
			values = dataGeneration(Byte.MIN_VALUE + 1, Byte.parseByte(constraint.get(Constraints.maxExclusive.toString())) - 1);
		} else if (constraint.containsKey(Constraints.maxInclusive.toString())) {
			values = dataGeneration(Byte.MIN_VALUE + 1, Byte.parseByte(constraint.get(Constraints.maxInclusive.toString())));
		} else {
